...
realization_stage_summary_plot <- function(
  shortname,
  ras_element,
  hms_element,
  usgs_id,
  ...
  all_stage_values <- c(
    unlist(lapply(ras_stage, function(x) sapply(x$stage, function(s) s$value))),
    unlist(lapply(hms_stage, function(x) sapply(x$stage, function(s) s$value))),
    unlist(lapply(ressim_stage, function(x) sapply(x$stage, function(s) s$value))),
    if (!is.null(obs_stage)) obs_stage$Stage else numeric(0)
  )

  # Remove NA values if any
  all_stage_values <- all_stage_values[!is.na(all_stage_values)]

  # Set dynamic y-axis limits
  y_min <- min(all_stage_values) * 0.9
  y_max <- max(all_stage_values) * 1.1
  ...
  fig <- plot_ly() %>%
    plotly::layout(
      title = list(
        text = paste0(
          "Stage Plot - All Realizations<br>",
      ...
      yaxis = list(title = "Water Surface Elevation (ft)", type = "linear", range = c(y_min, y_max),
  ...
  ras_realizations <- lapply(ras_stage, function(x) x$realization)
  hms_realizations <- lapply(hms_stage, function(x) x$realization)
  ressim_realizations <- lapply(ressim_stage, function(x) x$realization)
  ...
  for (real in all_realizations) {
    ...
    df_ras <- get_realization_df(ras_stage, real)
    # message(df_ras)
    ...
    # HMS
    df_hms <- get_realization_df(hms_stage, real)
    ...
    # ResSim
    df_ressim <- get_realization_df(ressim_stage, real)
  ...
  # Observed traces
  obs_stage <- get_obs_df(obs_stage)
  if (!is.null(obs_stage) && length(obs_stage) > 0) {
    fig <- fig %>%
      add_trace(x = obs_stage$Z, y = obs_stage$Stage, type = 'scatter', mode = 'markers',
                name = "Observed", legendgroup = "Observed", xaxis = "x2",
                hovertext = paste0("Stage: ", ifelse(is.numeric(obs_stage$Stage), round(obs_stage$Stage, 1), "Non-numeric value"), "<br>Date: ", obs_stage$Date),
